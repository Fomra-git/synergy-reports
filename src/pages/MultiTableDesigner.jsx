...
function normaliseSection(s) {
  return {
    ...
    rowFieldTransforms: { findText: '', replaceWith: '', simplifyDate: false, simplifyTime: false, normalizeMonth: false, normalizeWeek: false, ...(s.rowFieldTransforms || {}) },
    colFieldTransforms: { findText: '', replaceWith: '', simplifyDate: false, simplifyTime: false, normalizeMonth: false, normalizeWeek: false, ...(s.colFieldTransforms || {}) },
                                    ...
                                    {(col.rowFilters || []).map((f, fi) => (
                                      <div key={fi} style={{ padding: '10px', background: 'rgba(245,158,11,0.05)', borderRadius: '10px', border: '1px solid rgba(245,158,11,0.2)', display: 'flex', flexDirection: 'column', gap: '8px', position: 'relative', zIndex: (col.rowFilters?.length || 0) - fi }}>
                                        ...
                                        {f.type === 'expr_compare' ? (
                                          ...
                                          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                            <div><label style={{ fontSize: '9px', color: 'var(--text-muted)', display: 'block', marginBottom: '3px' }}>Date Column</label><SearchableDropdown options={masterHeaders} value={f.conditionCol || ''} onChange={v => updateColRowFilter(col.id, fi, 'conditionCol', v)} placeholder="Select date column..." /></div>
