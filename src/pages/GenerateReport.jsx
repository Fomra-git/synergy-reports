...
export default function GenerateReport() {
  ...
  // The multi-file merge config that applies to a run (first template with extra files wins).
  ...
  const processFile = async (explicitTemplateId = null) => {
    ...
    // Multi-file templates require every additional Excel file
    ...
    try {
      ...
      const generateScoreboardReport = async (template) => {
        ...
        const parseSbDate = (val) => {
          ...
          let s = cleanVal(val);
          // Numeric strings that are actually Excel date serials (e.g. a date column read as text)
          if (/^\d+(?:\.\d+)?$/.test(s)) {
