...
export default function CustomReport() {
  ...
  const [customFieldDefs, setCustomFieldDefs] = useState([]);   // [{ label, column }]
  const [customOptions, setCustomOptions] = useState({});        // { column: [distinct values] }
  const [customSelections, setCustomSelections] = useState({});  // { column: [selected values] }
  const [pendingTemplateId, setPendingTemplateId] = useState(null); // explicit template for single-download
  const [preparingPrompts, setPreparingPrompts] = useState(false);

  // Component-scope field-name normalizer (mirrors the one inside processFile)
  const ccCleanField = (str) =>
    String(str || '')
      .trim()
      .replace(/^["'\s]+|["'\s]+$/g, '')
      .replace(/[​-‍﻿]/g, '')
  ...
  // Collect the unique custom fields (by column) defined on the target templates
  const collectCustomFields = (targetTemplates) => {
    const map = new Map();
    targetTemplates.forEach(t => (t.customFields || []).forEach(cf => {
      if (cf && cf.column) {
        const key = ccCleanField(cf.column);
        if (key && !map.has(key)) map.set(key, { label: cf.label || cf.column, column: cf.column });
      }
    }));
    return [...map.values()];
  };
  ...
  const processFile = async (explicitTemplateId = null, selections = {}) => {
    ...
    try {
      ...
      // ── Custom Report: filter master data by the user's custom-field selections ──
      // Each entry is { column → [selected values] }. A row is kept only if, for every
      // field with a non-empty selection, the row's value in that column matches one of
      // the selected values. Applied here (before all context computations) so every
      // downstream template type — pivot, multi-table, scoreboard, fitness, visual
      // mapper — automatically respects the selection. Empty selections impose no filter.
      {
        const _ccPairs = Object.entries(selections || {})
          .filter(([, vals]) => Array.isArray(vals) && vals.length > 0)
          .map(([col, vals]) => [col, new Set(vals.map(v => String(v).trim().toLowerCase()))]);
        if (_ccPairs.length > 0) {
          setStatus('Applying custom field selections...');
          for (let i = masterData.length - 1; i >= 0; i--) {
            const row = masterData[i];
            const keep = _ccPairs.every(([col, valSet]) =>
              valSet.has(String(getMasterValue(row, col) ?? '').trim().toLowerCase())
            );
            if (!keep) masterData.splice(i, 1);
          }
        }
      }
      ...
      const evaluateCondition = (row, mapping) => {
        ...
        if (mapping.type === 'expr_compare') {
          ...
          let lhs = v1;
          const mathOp = mapping.mathOp || '-';
          if (mathOp === '+') lhs = v1 + v2;
          else if (mathOp === '-') lhs = v1 - v2;
          else if (mathOp === '*') lhs = v1 * v2;
          else if (mathOp === '/') lhs = v2 !== 0 ? v1 / v2 : 0;
        ...
        if (mapping.type === 'time_range') {
          const tv = parseTimeValue(getMasterValue(row, mapping.conditionCol));
        ...
        if (mapping.rules && mapping.rules.length > 0) return mapping.rules.every(r => {
          if (r.type === 'expr_compare') {
            ...
            let lhs = v1;
            const rMathOp = r.mathOp || '-';
            if (rMathOp === '+') lhs = v1 + v2;
            else if (rMathOp === '-') lhs = v1 - v2;
            else if (rMathOp === '*') lhs = v1 * v2;
            else if (rMathOp === '/') lhs = v2 !== 0 ? v1 / v2 : 0;
      ...
      const cleanValue = (val, config, colName) => {
        ...
        if (config.simplifyTime) {
          // If the raw value is a numeric Excel time/datetime fraction, convert to "H:MM AM/PM".
          // (Excel stores time-only cells as fractions 0–1; datetime cells as serial+fraction.)
          // For non-numeric strings like "Tue, Apr 15, 6:30 PM" keep the comma-split fallback.
          const _tv = typeof val === 'number' && !isNaN(val) ? val : parseFloat(cleaned);
          if (!isNaN(_tv) && _tv >= 0) {
            const _frac = _tv >= 1 ? _tv % 1 : _tv;       // extract time-of-day fraction
            if (_frac > 0 || _tv < 1) {                    // has a time component (skip pure integers ≥ 1)
        ...
        if (section.isFlatList && rowField) {
          ...
          if (section.mergeByCol) {
            ...
            flatRows = sorted.filter(r => {
              const gk = String(getMasterValue(r, rowField) || '').trim();
              const dk = String(getMasterValue(r, section.mergeByCol) || '').trim();
              if (!dk) return true;
              const key = `${gk}||${dk}`;
          ...
          if (template.type === 'pivot') {
            ...
            if (template.isFlatList && isPG) {
              ...
              if (template.mergeByCol) {
                ...
                flatRows = sorted.filter(r => {
                  const gk = String(getMasterValue(r, rowField) || '').trim();
                  const dk = String(getMasterValue(r, template.mergeByCol) || '').trim();
                  if (!dk) return true;
                  const key = `${gk}||${dk}`;
                  if (seen.has(key)) return false;
                  seen.add(key);
            ...
            for (const section of (template.sections || [])) {
              ...
              sectionInfos.push({ title: section.title || '', aoa: generatePivotSectionAOA(section, sectionData), flatListMergeFirstCol: !!(section.rowField && (section.isFlatList || section.mergePropertyCol)) });
            }
  ...
  const computeCustomOptions = async (fields, dualCfg = null) => {
    ...
    fields.forEach(f => {
      const set = new Set();
      rows.forEach(r => {
        const v = String(getMasterValue(r, f.column) ?? '').trim();
        if (v) set.add(v);
      });
      opts[f.column] = [...set].sort((a, b) => a.localeCompare(b, undefined, { numeric: true, sensitivity: 'base' }));
    });
    return opts;
  };

  // Entry point for the Generate buttons. If the selected templates define custom
  // fields, prompt the user to pick values first; otherwise generate immediately.
  const handleGenerate = async (explicitTemplateId = null) => {
    const targetTemplates = explicitTemplateId
      ? templates.filter(t => t.id === explicitTemplateId)
      : templates.filter(t => selectedTemplates.includes(t.id));
    if (!masterFile || targetTemplates.length === 0) return;
    ...
    try {
      ...
      setCustomSelections(prev => {
        const next = {};
        fields.forEach(f => { next[f.column] = prev[f.column] || []; });
        return next;
      });
      setPendingTemplateId(explicitTemplateId);
      setCustomModalOpen(true);
    } catch (e) {
      setError(`Could not read custom field values: ${e.message}`);
    } finally {
      setPreparingPrompts(false);
    }
  };

  const confirmCustomAndGenerate = () => {
    setCustomModalOpen(false);
    processFile(pendingTemplateId, customSelections);
  };
    ...
    return (
      <div className="generate-report">
        <header className="page-header">
          ...
          <p className="page-description">Generate reports with interactive custom-field prompts. Select a report category to get started.</p>
        </header>
    ...
    <div className="generate-report">
      <header className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          ...
          <p className="page-description">
            {selectedCategory?.description || 'Upload a master file, choose values for any custom fields, and generate a tailored report.'}
          </p>
        </div>
        {selectedCategory && (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(400px, 1fr) 320px', gap: '32px' }}>
               ...
               {filteredTemplates.map(template => (
                 ...
                 >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                      {masterFile && (
                        <button className="btn-link" onClick={(e) => { e.stopPropagation(); handleGenerate(template.id); }} disabled={isGenerating || preparingPrompts} style={{ color: 'var(--primary)', padding: '8px' }}><Download size={18} /></button>
                      )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
           <div className="glass" style={{ padding: '32px', textAlign: 'center' }}>
              ...
              <p style={{ color: 'var(--text-muted)', fontSize: '13px', marginBottom: '32px' }}>Generate {selectedTemplates.length} report{selectedTemplates.length !== 1 ? 's' : ''}. If a template defines custom fields, you'll be prompted to choose values first.</p>
              <button
                className="btn-primary btn-full"
                disabled={!masterFile || selectedTemplates.length === 0 || isGenerating || preparingPrompts}
                onClick={() => handleGenerate(null)}
                style={{ height: '56px', fontSize: '16px' }}
              >
                {isGenerating ? <><Loader2 className="spinner" size={20} /> Processing...</>
                  : preparingPrompts ? <><Loader2 className="spinner" size={20} /> Reading options...</>
                  : <><Download size={20} /> Generate Custom Report</>}
              </button>
      ...
      {customModalOpen && (
        <div
          ...
          style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', backdropFilter: 'blur(4px)', zIndex: 9998, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}
        >
          <div
            onClick={e => e.stopPropagation()}
            className="glass"
            style={{ width: '100%', maxWidth: '540px', maxHeight: '85vh', overflowY: 'auto', padding: '32px', borderRadius: '24px' }}
          >
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
              <h3 style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '18px', fontWeight: '700', margin: 0 }}>
                <MessageCircleQuestion size={20} color="var(--primary)" /> Custom Report Options
              </h3>
              <button onClick={() => setCustomModalOpen(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)' }}>
                <X size={20} />
              </button>
            </div>
            <p style={{ fontSize: '13px', color: 'var(--text-muted)', marginBottom: '24px', lineHeight: 1.5 }}>
              Choose the values to include for each field. Leave a field empty to include <strong>all</strong> of its values.
            </p>

            <div style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
              {customFieldDefs.map(f => (
                <div key={f.column}>
                  <label style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', fontSize: '13px', fontWeight: '600', marginBottom: '8px' }}>
                    <span>{f.label}</span>
                    <span style={{ color: 'var(--text-muted)', fontWeight: 400, fontSize: '12px' }}>
                      {(customSelections[f.column]?.length || 0)} of {(customOptions[f.column]?.length || 0)} selected
                    </span>
                  </label>
                  <MultiSelectCheckboxDropdown
                    options={customOptions[f.column] || []}
                    values={customSelections[f.column] || []}
                    onChange={vals => setCustomSelections(prev => ({ ...prev, [f.column]: vals }))}
                    placeholder={`All ${f.label} (no filter)`}
                  />
                </div>
              ))}
              {customFieldDefs.length === 0 && (
                <p style={{ fontSize: '13px', color: 'var(--text-muted)' }}>No custom fields configured for the selected template(s).</p>
              )}
            </div>
            ...
            <div style={{ display: 'flex', gap: '12px', marginTop: '28px' }}>
              <button
                onClick={() => setCustomModalOpen(false)}
                style={{ flex: 1, height: '48px', background: 'var(--glass-subtle)', border: '1px solid var(--border)', borderRadius: '14px', color: 'var(--text-main)', fontSize: '14px', fontWeight: 600, cursor: 'pointer' }}
              >
                Cancel
              </button>
              <button
                onClick={confirmCustomAndGenerate}
