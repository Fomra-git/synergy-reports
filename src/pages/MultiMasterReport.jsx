...
export default function MultiMasterReport() {
  ...
  const [masterFiles, setMasterFiles] = useState([]); // multiple master files (merged into one dataset)
  ...
  const isValidMasterFile = (file) =>
    !!file && (file.type === 'text/csv' || file.name.endsWith('.xlsx') || file.name.endsWith('.xls'));

  // Accepts one or more files; appends valid ones (de-duplicated by name+size) to the list.
  const handleFileChange = (e) => {
    const incoming = Array.from(e.target.files || []);
    if (incoming.length === 0) return;
    const valid = incoming.filter(isValidMasterFile);
    ...
    setError(invalidCount > 0 ? 'Some files were skipped. Please upload only Excel or CSV files.' : '');
  };

  const removeMasterFile = (idx) => {
    setMasterFiles(prev => prev.filter((_, i) => i !== idx));
  };
  ...
  const processFile = async (explicitTemplateId = null) => {
    ...
    if (masterFiles.length === 0 || targetTemplates.length === 0) return;

    setIsGenerating(true);
    setStatus(`Reading ${masterFiles.length} master file${masterFiles.length > 1 ? 's' : ''}...`);

    try {
      // Read every uploaded master file and concatenate their rows into one combined
      // dataset. Each file is parsed (and its merged cells resolved) independently — the
      // same way the single-file generator handles one file — then all rows are merged so
      // every downstream template (pivot, multi-table, scoreboard, visual mapper, etc.)
      // works unchanged against the union of all files.
      const masterData = [];
      for (let fi = 0; fi < masterFiles.length; fi++) {
        const file = masterFiles[fi];
        setStatus(`Reading file ${fi + 1} of ${masterFiles.length}: ${file.name}...`);
        const data = await file.arrayBuffer();
        const workbook = XLSX.read(data, { type: 'array' });
        const firstSheetName = workbook.SheetNames[0];
        const worksheet = workbook.Sheets[firstSheetName];
        const fileData = XLSX.utils.sheet_to_json(worksheet, { defval: "" });

        if (worksheet['!merges'] && fileData.length > 0) {
           ...
           worksheet['!merges'].forEach(range => {
              ...
              for (let c = s.c; c <= e.c; c++) {
                 ...
                 const sourceRow = fileData[firstDataRowIdx];
                 if (!sourceRow) return;
                 const val = sourceRow[headerName];
                 if (val === undefined || val === null || val === "") return;
                 for (let r = s.r; r <= e.r; r++) {
                    const targetRowIdx = r - 1;
                    if (targetRowIdx >= 0 && targetRowIdx < fileData.length) {
                       if (fileData[targetRowIdx][headerName] === "") {
                          fileData[targetRowIdx][headerName] = val;
                       }
                    }
                 }
              }
        ...
        // Append this file's rows to the combined dataset.
        fileData.forEach(row => { masterData.push(row); });
      }
    ...
    return (
      <div className="generate-report">
        <header className="page-header">
          <h1 className="page-title">Multi Master File Report</h1>
          <p className="page-description">Upload multiple master files at once — select a report category to get started.</p>
        </header>
      ...
      <header className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          ...
          <p className="page-description">
            {selectedCategory?.description || 'Upload multiple master files — their data is combined and processed through your selected templates.'}
          </p>
        </div>
        {selectedCategory && (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(400px, 1fr) 320px', gap: '32px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
          ...
          <div className="glass" style={{ padding: '32px' }}>
             ...
             <div
               className={`upload-zone ${isDragging ? 'dragging' : ''} ${masterFiles.length > 0 ? 'has-file' : ''}`}
               onClick={() => fileInputRef.current.click()}
               onDragOver={(e) => { e.preventDefault(); setIsDragging(true); }}
               onDragLeave={() => setIsDragging(false)}
               onDrop={(e) => { e.preventDefault(); setIsDragging(false); const files = Array.from(e.dataTransfer.files || []); if (files.length) handleFileChange({ target: { files } }); }}
               style={{ padding: masterFiles.length > 0 ? '32px' : '60px', border: '2px dashed var(--border)', borderRadius: '24px', textAlign: 'center', cursor: 'pointer', transition: '0.3s' }}
             >
                <input type="file" ref={fileInputRef} multiple onChange={handleFileChange} style={{ display: 'none' }} accept=".xlsx, .xls, .csv" />
                {masterFiles.length > 0 ? (
                  <div style={{ textAlign: 'left' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
                      <CheckCircle2 color="var(--success)" size={20} />
                      <p style={{ fontWeight: '700', fontSize: '15px' }}>{masterFiles.length} file{masterFiles.length > 1 ? 's' : ''} ready to process</p>
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      {masterFiles.map((f, idx) => (
                        <div key={`${f.name}-${idx}`} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 14px', background: 'var(--glass-subtle)', borderRadius: '12px', border: '1px solid var(--border)' }}>
                          <div className="login-logo-icon" style={{ width: '36px', height: '36px', background: 'rgba(16, 185, 129, 0.1)', color: 'var(--success)', flexShrink: 0 }}><FileSpreadsheet size={18} /></div>
                          <div style={{ flex: 1, overflow: 'hidden' }}>
                            <p style={{ fontWeight: '600', fontSize: '14px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{f.name}</p>
                            <p style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{(f.size / 1024).toFixed(0)} KB</p>
                          </div>
                          <button
                            onClick={(e) => { e.stopPropagation(); removeMasterFile(idx); }}
                            title="Remove file"
                            style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', fontSize: '20px', lineHeight: 1, padding: '4px 8px', flexShrink: 0 }}
                          >×</button>
                        </div>
                      ))}
                    </div>
                    <p style={{ fontSize: '12px', color: 'var(--primary)', marginTop: '14px', fontWeight: '500' }}>+ Click or drag to add more files</p>
                  </div>
                ) : (
                  <>
                    <div className="upload-icon"><Upload size={32} /></div>
                    <p style={{ fontWeight: '600', fontSize: '16px' }}>Click or drag multiple Excel/CSV files here</p>
                    <p style={{ fontSize: '13px', color: 'var(--text-muted)', marginTop: '8px' }}>All files are combined into one dataset. Your data never leaves your browser.</p>
                  </>
                )}
             </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
           <div className="glass" style={{ padding: '32px', textAlign: 'center' }}>
              ...
              <p style={{ color: 'var(--text-muted)', fontSize: '13px', marginBottom: '32px' }}>Combining {masterFiles.length} master file{masterFiles.length !== 1 ? 's' : ''} to generate {selectedTemplates.length} report{selectedTemplates.length !== 1 ? 's' : ''} in a single ZIP file.</p>
              <button
                className="btn-primary btn-full"
                disabled={masterFiles.length === 0 || selectedTemplates.length === 0 || isGenerating}
