...
export default function PivotTemplateManager() {
  ...
  const addFilter = (listType) => {
    setFormData(prev => ({
      ...prev,
      [listType]: [...(prev[listType] || []), { conditionCol: '', operator: '==', conditionVals: [], isManual: false, mode: 'filter' }]
    }));
  };
                      ...
                      {formData.globalFilters.map((f, i) => (
                        <div key={i} className="glass" style={{ padding: '12px', borderRadius: '12px', border: '1px solid var(--border)', position: 'relative' }}>
                          ...
                          <div style={{ display: 'flex', gap: '4px', marginBottom: '4px' }}>
                            <button onClick={() => updateFilter('globalFilters', i, 'mode', 'filter')} style={{ padding: '2px 8px', fontSize: '10px', borderRadius: '6px', border: '1px solid var(--border)', background: f.mode !== 'exclude' ? 'var(--primary)' : 'var(--glass-bg)', color: f.mode !== 'exclude' ? 'white' : 'var(--text-muted)', cursor: 'pointer', fontWeight: '700' }}>Filter Rows</button>
                            <button onClick={() => updateFilter('globalFilters', i, 'mode', 'exclude')} style={{ padding: '2px 8px', fontSize: '10px', borderRadius: '6px', border: '1px solid var(--border)', background: f.mode === 'exclude' ? '#CD5C5C' : 'var(--glass-bg)', color: f.mode === 'exclude' ? 'white' : 'var(--text-muted)', cursor: 'pointer', fontWeight: '700' }}>Exclude Column</button>
                          </div>

                          <div className="form-group" style={{ marginBottom: '8px' }}>
                            <SearchableDropdown options={masterHeaders} value={f.conditionCol} onChange={v => updateFilter('globalFilters', i, 'conditionCol', v)} placeholder={f.mode === 'exclude' ? 'Select column to exclude...' : 'Select Field...'} />
                          </div>
          ...
          <div style={{ flex: 1, padding: "24px 32px 250px 32px", overflowY: "auto", display: "flex", flexDirection: "column", gap: "24px" }}>
             ...
             {formData.sortConfig?.enabled && (
               <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: '12px', padding: '12px 16px', background: 'var(--glass-subtle)', borderRadius: '10px', border: '1px solid var(--border)' }}>
                 <div className="form-group" style={{ marginBottom: 0 }}>
                   <label style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '4px' }}>Sort by Column</label>
                   <SearchableDropdown
                     options={[
                       formData.rowField || '',
                       ...((formData.pivotColumns || []).map(p => p.displayName || p.source || '').filter(Boolean))
                     ].filter(Boolean)}
                 ...
                 <div className="form-group" style={{ marginBottom: 0 }}>
                   <label style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '4px' }}>Direction</label>
                   <select
                 ...
                 <div className="form-group" style={{ marginBottom: 0 }}>
                   <label style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '4px' }}>Sort Type</label>
                   <select
                              ...
                              {(col.rowFilters || []).map((f, fi) => (
                                <div key={fi} style={{ display: 'flex', flexDirection: 'column', gap: '8px', padding: '10px', background: 'rgba(245,158,11,0.05)', borderRadius: '10px', border: '1px solid rgba(245,158,11,0.2)', position: 'relative', zIndex: (col.rowFilters?.length || 0) - fi }}>
                                  ...
                                  {f.type === 'repeat_visit' ? (
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                      <div>
                                        <label style={{ fontSize: '9px', color: 'var(--text-muted)', display: 'block', marginBottom: '3px' }}>Date Column</label>
                                        <SearchableDropdown options={masterHeaders} value={f.conditionCol || ''} onChange={v => updateColRowFilter(col.id, fi, 'conditionCol', v)} placeholder="Select date column..." />
                                      </div>
                                      <div>
                                      ...
                                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                        <label style={{ fontSize: '9px', color: 'var(--text-muted)', whiteSpace: 'nowrap' }}>Min visits/day:</label>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                      <div>
                                        ...
                                        <SearchableDropdown options={masterHeaders} value={f.conditionCol || ''} onChange={v => updateColRowFilter(col.id, fi, 'conditionCol', v)} placeholder="e.g. Appt Type..." />
                                      </div>
                                      <div>
                                        <label style={{ fontSize: '9px', color: 'var(--text-muted)', display: 'block', marginBottom: '3px' }}>Client / Patient ID Column</label>
                                        <SearchableDropdown options={masterHeaders} value={f.clientCol || ''} onChange={v => updateColRowFilter(col.id, fi, 'clientCol', v)} placeholder="Select client column..." />
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                      <div>
                                        <label style={{ fontSize: '9px', color: 'var(--text-muted)', display: 'block', marginBottom: '3px' }}>Column</label>
                                        <SearchableDropdown options={masterHeaders} value={f.conditionCol || ''} onChange={v => updateColRowFilter(col.id, fi, 'conditionCol', v)} placeholder="Select time column..." />
                                      </div>
                                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px' }}>
                                        <div>
                                          <label style={{ fontSize: '9px', color: 'var(--text-muted)', display: 'block', marginBottom: '3px' }}>From</label>
                                          <input type="time" value={f.conditionVals?.[0] || ''} onChange={e => updateColRowFilter(col.id, fi, 'conditionVals', [e.target.value, f.conditionVals?.[1] || ''])} style={{ width: '100%', padding: '7px 6px', fontSize: '12px', boxSizing: 'border-box' }} />
                                        </div>
                                        <div>
                                          <label style={{ fontSize: '9px', color: 'var(--text-muted)', display: 'block', marginBottom: '3px' }}>To</label>
                                          <input type="time" value={f.conditionVals?.[1] || ''} onChange={e => updateColRowFilter(col.id, fi, 'conditionVals', [f.conditionVals?.[0] || '', e.target.value])} style={{ width: '100%', padding: '7px 6px', fontSize: '12px', boxSizing: 'border-box' }} />
                                        </div>
                                      </div>
                                    </div>
                                  ) : (
